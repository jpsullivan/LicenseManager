...
namespace LicenseManager.Infrastructure.Attributes
{
    ...
    public class IntraRouteAttribute : ActionMethodSelectorAttribute, IComparable<IntraRouteAttribute>
    {
        ...
        public static void MapDecoratedRoutes(RouteCollection routes, Assembly assemblyToSearch)
        {
            var decoratedMethods = from t in assemblyToSearch.GetTypes()
                                   ...
                                   where m.IsDefined(typeof(IntraRouteAttribute), false)
                                   select m;
            ...
            var methodsToRegister = new SortedDictionary<IntraRouteAttribute, MethodInfo>(); // sort urls alphabetically via RouteAttribute's IComparable implementation

            // first, collect all the methods decorated with our RouteAttribute
            foreach (var method in methodInfos)
            {
                foreach (var attr in method.GetCustomAttributes(typeof(IntraRouteAttribute), false))
                {
                    var ra = (IntraRouteAttribute)attr;
        ...
        public IntraRouteAttribute(string url)
            : this(url, "", null, RoutePriority.Default, RouteHandler.Mvc)
        {
        }

        public IntraRouteAttribute(string url, HttpVerbs verbs)
        ...
        public IntraRouteAttribute(string url, HttpVerbs verbs, RoutePriority priority)
        ...
        public int CompareTo(IntraRouteAttribute other)
        {
